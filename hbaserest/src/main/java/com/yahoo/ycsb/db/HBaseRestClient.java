/**
 * Copyright (c) 2010 Yahoo! Inc. All rights reserved. Licensed under the Apache License, Version
...
/**
 * HBase Rest client for YCSB framework
 */
public class HBaseRestClient extends com.yahoo.ycsb.DB {
  ...
  public void getHTable(String table) throws IOException {
    synchronized (tableLock) {
      ...
      List<String> restServers = Arrays.asList(config.get("hbase.rest.servers").split(","));
      Cluster cluster = new Cluster();
      for (String node : restServers) {
        cluster.add(node);
      }
      Client client = new Client(cluster);

      // initialize htable connection for accessing HBase table
      _hTable = new RemoteHTable(client, config, table);
      ...
      Thread t = new Thread() {
        public void run() {
          try {
            ...
            HBaseRestClient cli = new HBaseRestClient();
            ...
            for (int i = 0; i < opcount; i++) {
               ...
               * hm.put("field3","value3"); rescode=cli.insert("table1",key,hm); HashSet<String> s =
               * new HashSet(); s.add("field1"); s.add("field2"); rescode=cli.read("table1", key, s,
               * result); //rescode=cli.delete("table1",key); rescode=cli.read("table1", key, s,
               * result);
               */
